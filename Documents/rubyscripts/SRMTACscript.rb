...
Dir.glob('*SRM*.json').each do |filename|
  ...
  puts parsed
  puts "-------"
  parsed["APCriticalChanceMultiplier"] = ((parsed["Damage"] * 0.15) / 40).ceil(3) 
  parsed["APMaxArmorThickness"] = (parsed["Damage"] * 5 ).ceil(3) 
  parsed["APArmorShardsMod"] = 0.35
  puts parsed
  puts "-------"
